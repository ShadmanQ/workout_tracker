        ...
        if (len(os.listdir('food_history'))) > 0:
                for file in os.listdir('food_history'):
                    with open('./food_history/'+file,"r",encoding='utf-8') as openfile:
                        food_history[file.split("_")[0]]=json.load(openfile)
    else:
...
def add_a_meal(f_history):
    food = get_nutrition_info(input("what did you eat?"))
    ...
    if stwing not in f_history:
        f_history[stwing] = []
    f_history[stwing].append(((da_keys[choice-1],food[da_keys[choice-1]])))

def export_food(food_history):
    print("now exporting!")
    print(food_history)
    for i in food_history:
        with open('./food_history/'+i+"_food.json","w",encoding='utf-8') as f_write:
            json.dump(food_history[i],f_write)
        f_write.close()
